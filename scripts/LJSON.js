...
var parsenhora = require(["scripts/parsenhora.js"]);
      ...
      function application(varName, argList) {
        var app = function (arg) {
          if (arg === null) {
            ...
            return (
              varName +
              (argList.length === 0 ? "" : argList.map(stringifyCall).join(""))
            );
          } else {
        ...
        parsenhora(function (P) {
          ...
          function LJSON_number(binders, scope) {
            return function () {
              ...
              if (e !== null) {
                result += "e";
                if (P.chr("+")() !== null) result += "+";
                else if (P.chr("-")() !== null) result += "-";
          ...
          function LJSON_lambda(binders, scope) {
            return function () {
              ...
              var body = P.betweenSpaced(
                P.chr("("),
                LJSON_value(binders + varList.length, newScope),
                P.chr(")")
              )();
              var args = varList
                .map(function (name, i) {
                  return toName(binders + i);
                })
                .join(",");
