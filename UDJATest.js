class UDJATest {
  constructor() {
    ...
    this.types = [
      ["ESTJ", [245, 68, 40],
        [245, 68, 40], "Te Si Ne Fi", "Delta"
      ],
      ["ESTP", [63, 78, 175],
        [65, 86, 160], "Se Ti Fe Ni", "Beta"
      ],
      ["ENTJ", [107, 175, 24],
        [107, 175, 24], "Te Ni Se Fi", "Gamma"
      ],
      ["ENFJ", [210, 224, 56],
        [238, 201, 58], "Fe Ni Se Ti", "Beta"
      ],
      ["ESFJ", [38, 147, 62],
        [58, 150, 46], "Fe Si Ne Ti", "Alpha"
      ],
      ["ESFP", [255, 73, 165],
        [255, 73, 143], "Se Fi Te Ni", "Gamma"
      ],
      ["ENTP", [223, 81, 255],
        [129, 41, 145], "Ne Ti Fe Si", "Alpha"
      ],
      ["ENFP", [98, 146, 196],
        [124, 164, 201], "Ne Fi Te Si", "Delta"
      ],
      ["ISTJ", [126, 200, 211],
        [93, 197, 198], "Si Te Fi Ne", "Delta"
      ],
      ["ISTP", [245, 110, 28],
        [245, 110, 28], "Ti Se Ni Fe", "Beta"
      ],
      ["INTJ", [255, 109, 255],
        [219, 66, 241], "Ni Te Fi Se", "Gamma"
      ],
      ["INFJ", [134, 129, 242],
        [39, 123, 177], "Ni Fe Ti Se", "Beta"
      ],
      ["ISFJ", [255, 112, 195],
        [248, 149, 224], "Si Fe Ti Ne", "Alpha"
      ],
      ["ISFP", [108, 245, 55],
        [108, 245, 55], "Fi Se Ni Te", "Gamma"
      ],
      ["INTP", [88, 248, 143],
        [88, 248, 143], "Ti Ne Si Fe", "Alpha"
      ],
      ["INFP", [250, 174, 73],
        [250, 174, 73], "Fi Ne Si Te", "Delta"
      ],
    ];
    ...
    this.attributes = [
      ...
      ['Initiating', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Responding', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Te Fi', [150, 150, 150],
        [150, 150, 150]
      ...
      ['Affiliative', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Pragmatic', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Systematic', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Interest', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Abstract', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Concrete', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Crusader', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Philosopher', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Templar', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Wayfarer', [150, 150, 150],
        [150, 150, 150]
      ],
      ['Soul', [150, 150, 150],
        [150, 150, 150]
    ...
    let width2 = this.w / 2;
    let y2 = this.y + 4 * (height + this.boundry_offset) + this.boundry_offset * 3;
    let height2 = this.h - (y2 - this.y) - height * .75;
    ...
    for (let row = 0; row < 4; row++) {
      for (let col = 0; col < 2; col++) {
        this.buttons.push(new button_attributes(
          this.x, this.y, this.w, this.h,
          this.boundry_offset, row, col,
          this.attributes));
      }
    }
    for (let row = 4; row < 6; row++) {
      let col = 0;
      this.buttons.push(new button_attributes_four(
        this.x, this.y, this.w, this.h,
        this.boundry_offset, row, col,
        this.attributes));
    }
  ...
  draw() {
    ...
    let y2 = this.y + 4 * (height + this.boundry_offset) + this.boundry_offset * 3;
    height = this.h - (y2 - this.y) - height * .75;
    ...
    textSize(min(height * .5, width / 10));
    textFont("Impact");
    // text(
    //   'Submit',
  ...
  mousePressed(x, y) {
    ...
    if ((this.x <= x && x <= this.x + this.w) &&
      (this.y <= y && y <= this.y + height * 4 + this.boundry_offset * 3)) {
      let posx = x - this.x - 2;
      let posy = y - this.y - 2;

      if ((posx % (width + this.boundry_offset) <= width) &&
    ...
    if ((this.x <= x && x <= this.x + this.w) &&
      (y2 <= y && y <= y2 + height * 6 + this.boundry_offset * 5)) {
      let posx = x - this.x - 2;
      let posy = y - y2 - 2;
    ...
    if ((x_obj <= x && x <= x_obj + width) &&
      (y_obj <= y && y <= y_obj + height)) {
...
class button_attributes {
  ...
  draw_left_text(y2, width, height) {
    push();
    let shake = (Date.now() - this.attributes[(this.col * 4 + this.row) * 2][3]) < 250;
    if (shake) {
  ...
  draw_right_text(y2, width, height) {
    push();
    let shake = (Date.now() - this.attributes[(this.col * 4 + this.row) * 2 + 1][3]) < 250;
    if (shake) {
      translate(random(-5, 5), random(-5, 5));
    }
    noStroke();
    ...
    text(
      ...
      this.x + this.col * (width + this.boundry_offset) - this.boundry_offset / 2,
      y2 + this.row * (height + this.boundry_offset) + height / 2,
      width
    );
    pop();
